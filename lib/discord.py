...
def send_trade_book(books, discord_webhook) -> None:
    """
    send_trade_book takes a list of dictionaries and builds an ascii table, open percent profit summary and sends to discord.

    :param books: List of books where each book is a dictionary with symbol as key, book details as value
    :param discord_webhook: The Discord webhook URL for the current book
    :type books: List[Dictionary]
    :type discord_webhook: String
    :return: None
    :rtype:
    """
...
async def delayed_send_trade_book(book, discord_webhooks):
    while conf.discord_webhook_config_1_enabled:
        await sleep(conf.trade_book_wait)
        if len(book) > 0:
            send_trade_book(book, discord_webhooks)
